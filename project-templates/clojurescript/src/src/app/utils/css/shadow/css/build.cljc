(ns app.utils.css.shadow.css.build
  (:require
    [app.utils.css.shadow.css.specs :as s]
    [app.utils.css.shadow.css.analyzer :as ana]
    [clojure.set :as set]
...
#?(:clj
   (do (defn clj-file? [filename]
       ...
       (defn load-default-aliases-from-classpath [build-state]
         (update build-state :aliases merge-left (edn/read-string (slurp "src/app/utils/css/shadow/css/aliases.edn"))))

       (defn load-colors-from-classpath [build-state]
         (update build-state :colors merge-left (edn/read-string (slurp "src/app/utils/css/shadow/css/colors.edn"))))

       ;; (defn load-preflight-from-classpath [build-state]
       ;;   (assoc build-state
       ;;     :preflight-src
       ;;     (slurp (io/resource "src/app/utils/css/shadow/css/preflight.css"))))
