...
(defun +MM|npm-start-kits-app ()
  (interactive)
  (npm-mode)
  (let* ((dir (f-join (+my/project-root) "src/apps/"))
         (apps (--> dir
                    (f-entries it)
...
(setq +MM-import:package-exports
      (ht ("@meisterlabs/common"
           '("Try" "Maybe" "Either" "Box" "Switch" "Num" "Obj" "Arr" "Str" "Branch" "Color" "tap" "anyPass" "allPass"))
...
(defun +MM-import|import-other-window-file ()
  ...
  (let* ((current-file (buffer-file-name))
         (other-window-file (save-window-excursion (other-window -1) (buffer-file-name)))
         (file (if (string= (f-dirname current-file)
                            (f-dirname other-window-file))
                   (->> (f-base other-window-file)
                     (s-concat "./"))
