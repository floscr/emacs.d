...
#+BEGIN_SRC elisp
(defun +MM/find-bookmark-buffer (name &optional other-window?)
  ...
  (-when-let* ((find-fn (if other-window? 'find-file-other-window 'find-file))
               (item (--find (string= name (plist-get it :name)) +MM-dynamic-bookmarks))
...
(defun +MM/find-bookmark-buffer-other-window (item)
  (+MM/find-bookmark-buffer item t))
...
(after! ivy
  (ivy-set-actions
   '+MM|bookmarks
   '(("j" +MM/find-bookmark-buffer-other-window "open in other window"))))
#+END_SRC
*** Build Package
