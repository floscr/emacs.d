...
(defun +MM|npm-start-kits-app (&optional arg)
  (interactive "P")
  (npm-mode)
  (let* ((dir (or (->> (f-join (my-file/project-root) "src/apps/")
                       (-id-when #'f-exists?))
    ...
    (ivy-read "Start App: " apps
              :action (lambda (x)
                        (let ((cmd (t! "start <<x>> -- --port 3008"))
                          ...
                          (if arg
                              (my-shell/async-command-no-window (t! "alacritty --hold --working-directory <<root>> -e \"npm run <<cmd>>\""))
...
To commit call +MM|kits-package-update."
  ...
  (-when-let* ((json-encoding-pretty-print t)
               (file (->> (my-magit/list-modified-files "master")
...
(defun +MM|kits-package-update ()
  ...
  ;; (when (my-magit/check-for-merge-conflicts)
...
When SAME-BRANCH? is t use the current branch instead of creating a new one."
  (let* ((origin-path (projectile-project-root))
         (path (if same-branch?
                   (-if-let (branch (magit-get-current-branch))
                       (progn (magit-checkout (magit-rev-parse "HEAD"))
                              (my-magit|create-worktree-project-from-branch branch branch t))
                     (user-error "Error: No branch checked out!"))
                   (call-interactively #'my-magit|create-worktree-project-from-branch)))
    ...
    (my-shell/async-command-no-window (template "rsync -a <<(f-join origin-path \"node_modules\")>> <<(f-join path \"node_modules\")>>"))))
...
(defun +MM|write-rule (rule &optional prop)
  (let* ((maybe-prop-int (condition-case nil
         ...
         (prop (cond (maybe-prop-int maybe-prop-int)
                     ...
                     ;; String
                     (t (template "'<<prop>>'"))))
         (has-trailing-semi (my-buffer/line-contains ",$")))
...
(defun +MM/setup-keybindings ()
  ...
  (map! :map +MM-Web-mode-map
        ...
        (:prefix ("f" . "file")
         :desc  "Jump to Entity"               "e"  (cmd! (my-counsel|project-file-jump "src/containers/MapEditor/Entities"))
...
#+begin_src elisp
(defun +penpot|apply-dev-files ()
  "Applies my local modifications in the DEV stash and assumes them as unchanged."
  (interactive)
  (magit-git-command "git stash apply stash^{/DEV}")
  (dolist (file (magit-changed-files "HEAD"))
    (magit-git-command (format "git update-index --assume-unchanged %s" file))))
#+end_src
