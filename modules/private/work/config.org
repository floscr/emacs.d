...
(defun +MM|build-server ()
  ...
  (let* ((web-directory (or (projectile-project-root) +MM:mm-web-dir))
         ...
         (item (ivy-read "Build Package: " scripts :action))
         (command (--> (--find (string= (car it) item) scripts)
                       (plist-get (cdr it) :command))))
    (let* ((default-directory web-directory))
      (deferred:$
        (deferred:nextc it `(lambda () (deferred:process "mm-build-bundle" ,command)))))))
...
(defun +MM|kits-package-update ()
  ...
  (interactive)
  (-if-let* ((package-json (car (magit-staged-files)))
...
When SAME-BRANCH? is t use the current branch instead of creating a new one."
  (let* ((origin-path (projectile-project-root))
    ...
    (async-shell-command-no-window (template "cp -r <<(f-join origin-path \"node_modules\")>> <<(f-join path \"node_modules\")>>"))))
