...
(defun +MM|goto-action ()
  ...
  (interactive)
  (-if-let* ((action-str (--> (thing-at-point 'line t)
                              (s-match ".*: '\\(.*\\)\\.\\(.*\\)'.*" it)))
             (action-parent-id (nth 1 action-str))
             (action-function-name (nth 2 action-str))
      ...
      (-->
       (cond
        ((s-starts-with? "Canvas" action-parent-id)
         (--> (concat action-parent-id "." action-function-name)
              (s-replace "Canvas." "" it)
              (s-replace "." "/" it)
              (f-join src-dir "containers/MapEditor/actions" (template "<<it>>.js"))))

        ((s-contains? action-parent-id app-file-contents)
         (-->
          (s-match (template "<<action-parent-id>>.*require('\\(.*\\)')") app-file-contents)
          (nth 1 it)
          (s-replace "/controller" "" it)
          (f-join src-dir it "actions" (template "<<action-function-name>>.js")))))
