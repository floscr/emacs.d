...
(defadvice! +org/custom-archive-check (orig-fn &rest args)
  ...
  (let ((path
         (cond ((eq buffer-name org-agenda-buffer-name)
                (-some->> (org-get-at-bol 'org-marker)
                  (marker-buffer)
                  (buffer-file-name)))
               (t buffer-file-name)))))
  (if (f-ancestor-of? (f-join org-directory "Work") buffer-file-name)
      (let ((org-archive-location (+org/custom-archive-dir-str)))
        (apply orig-fn args))
    (apply orig-fn args)))
#+END_SRC
