 ...
 '("x" "Todo Items"
   ((agenda
     ...
     ((org-agenda-span 3)
      ...
      (org-super-agenda-groups '((:name "Work Meetings" :and (:tag ("MEETING") :not (:tag ("REPEATING"))))
                                 (:name nil :discard (:tag "WORK"))
...
(defun +org/org-set-active-state (&optional args args2)
  ...
  (cond ((buf-line-has "PROJECT") nil)
        ...
        ((not (buf-line-has (rx (or "ACTIVE" "NEXT" "DONE" "TODO" "WAITING" "SOMEDAY" "CANCELLED" "PROJECT")))) nil)
        (t (org-todo "ACTIVE"))))
...
#+BEGIN_SRC elisp
(defvar +org-archive:link-archive-directory nil
  "Directory where the backup files are stored.")
(setq +org-archive:link-archive-directory (+org/expand-org-file-name "LinkArchives"))

(defun +org-archive/archive-path (url &optional title)
...
Take the host as the base and either the TITLE or the path from the URL."
  (let* ((url-obj (url-generic-parse-url url))
    ...
    (f-join +org:link-archive-directory host path)))

(defun +org-archive/create-directory (url &optional title)
  "Create archive target directory from URL and TITLE."
  (interactive)
  (let ((path (+org-archive/archive-path url title)))
    (shell-command-to-string (concat "mkdir -p " path))
    path))

(defun +org-archive/wget-link (dir)
  "Wget the given URL to the +org-archive:link-archive-directory."
  (require 'deferred)
...
(defun +org-archive/convert-html-to-org (path)
...
(defun +org-archive/convert-html-to-epub (path)
  "Convert html files at PATH to org documents."
  (--> (f-entries path)
       (--filter (f-ext? it "html") it)
       (-first-item it)
       (deferred:process
         "ebook-convert"
         it
         "-o" (f-swap-ext it "epub"))))

(defun +org-archive|archive-dwim (&optional to-epub?)
  "Archive the first link under the cursor and set the property."
  (interactive)
  (require 'deferred)
  (require 'om)
  (let* ((header (om-parse-this-headline))
         ...
         (dir (+org-archive/create-directory url title)))
    (org-set-property "ARCHIVE" (template "[[<<dir>>]]"))
    (deferred:$
      (+org-archive/wget-link dir)
      (deferred:nextc it `(lambda ()
                            (+org-archive/convert-html-to-org ,dir)))
      (deferred:nextc it `(lambda ()
                            (when to-epub?
                              (+org-archive/convert-html-to-epub ,dir)))))))

(defun +org-archive|archive-dwim-epub ()
  "Function docstring"
  (interactive)
  (+org-archive|archive-dwim t))
#+END_SRC
...
(defun +org|narrow-subtree-indirect (&optional visit-fn)
  ...
  (let ((buffer
    ...
    (with-current-buffer buffer
      (widen)
      (setq header-line-format "Edit, then exit with 'C-c C-c'")
...
(defun +org-my-init-keybinds-h ()
  (map! :map org-mode-map
        ...
        (:prefix-map ("y" . "Yank")
         :desc "Item" "l" (λ! (copy-message (+org|link-url-from-point)))
         :desc "Item" "t" (λ! (copy-message (+org|link-title-from-point))))  
...
(defun +org-my-init-agenda-keybinds-h ()
  (map! :after org-agenda
        ...
        (:prefix ("f" . "Filter by")
         :desc "Tasks"               "t" (λ! (+org/agenda-set-tag-filter '("+TASK")))
         :desc "Reviews"             "r" (λ! (+org/agenda-set-tag-filter '("+REVIEW"))))
        (:prefix ("w" . "Refile")
...
(map!
 ...
 (:prefix-map ("n" . "Notes")
   ...
   (:prefix-map ("b" . "Bookmarks")
    :desc "Goto Bookmarks File"       "b" (λ! (find-file +org:bookmarks-file))
