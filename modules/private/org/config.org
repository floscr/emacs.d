...
(defun +org|add-created-property-only-todo ()
  "Add CREATED property with the current time to the current item."
  (interactive)
...
#+BEGIN_SRC elisp
(advice-add '+org/insert-item-below :after (η #'+org|add-created-property-only-todo))
(advice-add '+org/insert-item-above :after (η #'+org|add-created-property-only-todo))
#+END_SRC
