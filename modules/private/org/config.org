...
(+org/add-to-agenda-custom-commands
 '("w" "Work Agenda"
   ((agenda "a" ((org-agenda-sorting-strategy '(time-up priority-down todo-state-up scheduled-up user-defined-up))
                 (org-agenda-span 3)
...
(defun +org-ql|projects ()
  ...
  (interactive)
  (org-ql-search (+org/expand-org-file-names '("Main/projects.org"))
    '(and (todo)
          (ancestors
           (and (todo))))
...
(defun +org-ql|work-projects ()
  "List all work projects."
  (interactive)
  (org-ql-search (+org/expand-org-file-names '("Work/work.org"))
    '(and (todo)
          (ancestors
           (and (todo))))
    :sort '(todo priority date)
...
#+BEGIN_SRC elisp
(defun +org|search-programming-notes ()
...
(defun +org|narrow-subtree-indirect (&optional visit-fn goto-parent?)
...
(defun +org|clock-visit-entry (&optional arg)
  "Visit currently clocked org entry in a narrowed indirect buffer."
  (interactive "P")
  (let ((org-agenda-follow-indirect t))
    (+org|narrow-subtree-indirect #'org-clock-goto arg)))

(defun +org|agenda-visit-entry (arg)
  "Visit agenda entry in a narrowed indirect buffer."
  (interactive "P")
  (+org|narrow-subtree-indirect #'org-agenda-switch-to arg))
#+END_SRC
...
#+BEGIN_SRC elisp
(advice-add '+org/insert-item-below :after (η #'+org|add-created-property))
(advice-add '+org/insert-item-above :after (η #'+org|add-created-property))
#+END_SRC
...
(map! :map evil-org-mode-map
 ...
 :n "C-c C-l" (λ! (+evil-org/normal-mode-paste-fix #'org-insert-link)))
...
(map!
 ...
 (:prefix-map ("n" . "Notes")
   ...
   :desc "Visit Entry"              "SPC" #'+org|clock-visit-entry
   ...
   (:prefix-map ("r" . "Roam")
    ...
    :desc "Journal: New Entry" "j"     (λ! (let ((+org-roam-open-buffer-on-find-file nil))
                                              (call-interactively #'org-journal-new-entry)))
    :desc "List (Main)" "l" #'org-roam
    :desc "Insert" "i" (λ! (+evil-org/normal-mode-paste-fix #'org-roam-insert))
   ...
   (:prefix-map ("c" . "clock")
     ...
     :desc "Visit Entry"              "SPC" #'+org|clock-visit-entry
     :desc "Visit Entry"              "'"   #'+org|clock-visit-entry)))
#+END_SRC
*** Local Leader
**** Org Mode
...
(map! :map org-mode-map
      ...
      :desc  "Paste Chrome Link"         "p"  (λ! (+evil-org/normal-mode-paste-fix #'+org|paste-chrome-link))
      ...
      (:prefix-map ("y" . "Yank")
       :desc "Link" "l" (λ! (+my/copy-message (+org|link-from-point)))
       :desc "Url" "u" (λ! (+my/copy-message (+org|link-url-from-point)))
       :desc "Title" "t" (λ! (+my/copy-message (+org|link-title-from-point))))
      ...
      (:prefix-map ("i" . "Insert")
       ...
       :desc "Inavtive Timestamp" "i" (λ! (+evil-org/normal-mode-paste-fix #'org-time-stamp-inactive))
      ...
      (:prefix-map ("w" . "Refile")
       ...
       :desc "To GTD Backlog"     "b" (cmd! (+org/refile "Main/gtd.org" "GTD Backlog"))
